...
template_path_prefix = 'templates/NoEgo/' # The path prefix for the number templates
# Set crop zones for the desktop version
zone = {"top": 685, "left": 1190, "width": 175, "height": 175} # The zone around the bar
number_zone = {"top": 90, "left": 73, "width": 37, "height": 37} # The zone around the number
# Set colour ranges for the bar for the desktop version
blue_bar_lower = np.array([6, 2, 160], dtype="uint8") # The minimum HSV threshold for the blue target bar
blue_bar_upper = np.array([173, 40, 236], dtype="uint8") # The maximum HSV threshold for the blue target bar
...
while True:    
    ...
    if number == 0 or area != prev_area: # If the number is not set or the area has changed
        number_crop = frame[number_zone["top"]:number_zone["top"]+number_zone["height"],number_zone["left"]:number_zone["left"]+number_zone["width"]] # Crop the number zone from the frame
